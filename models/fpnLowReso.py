...
class fpn_module(nn.Module):
    def __init__(self, numClass):
        ...
        # Classify layers
        self.classify = nn.Conv2d(512, numClass*(4**2), kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, c2, c3, c4, c5):
        ...
        #
        # # Classify
        # lastFeature = F.upsample(self._concatenate(p5, p4, p3, p2), size=(612, 612), mode='bilinear')
        # output = self.classify(self.deconvlotion(lastFeature))

        output = self.pixel_shuffle(self.classify(self._concatenate(p5, p4, p3, p2)))
